...
export default {
        ...
        <div v-if="isOpen" class="fixed inset-0 bg-black/80 flex items-center justify-center z-50" @click.self="$emit('close')">
            <div class="bg-gray-800 rounded-lg border border-gray-700 p-6 w-full max-w-2xl h-[500px] flex flex-col">
                ...
                <div class="flex-1 overflow-y-auto space-y-2">
                    <div v-for="(mission, id) in displayedMissions" :key="id"
    ...
    setup(props, { emit }) {
        ...
        const displayedMissions = computed(() => {
            if (!availableMissions.value) return {};
            const result = {};
            for (const [id, mission] of Object.entries(availableMissions.value)) {
                // Hide NPC missions from the board unless active or completed
                const isNpcOnly = mission.source === 'npc';
                const isActive = player.value?.active_mission_id === id;
                const isCompleted = player.value?.completed_missions?.includes(id);

                if (!isNpcOnly || isActive || isCompleted) {
                    result[id] = mission;
                }
            }
            return result;
